...
class TaxBracketModelSim:
    def __init__(self, data):
        ...
        self.max_value = Decimal(str(data['max_value'])) if data.get('max_value') is not None else None
...
def get_applicable_bracket(tax_type, value, reference_date):
    ...
    if not table:
        raise ValueError(f"No active tax table found for {tax_type} on {reference_date}")
    ...
    raise ValueError(f"No applicable tax bracket found for {tax_type} with value {value} on {reference_date}")

def get_minimum_salary(reference_date):
    table = get_active_table("INSS", reference_date)
    if not table:
        raise ValueError(f"No active INSS tax table found for {reference_date} to determine minimum salary.")
    ...
    if lowest_bracket.get('max_value') is None:
        raise ValueError(f"Invalid INSS table configuration for {reference_date}: missing lowest bracket max_value.")
...
def calculate_inss(pro_labore_amount, reference_date):
    ...
    if not brackets:
        raise ValueError(f"No applicable INSS bracket found for {pro_labore_amount} on {reference_date}")
