...
from .models import Invoice, InvoiceItem, EmailTemplate, InvoiceTemplate, InvoiceTemplateItem
from django.forms import inlineformset_factory

VALID_INVOICE_MACROS = {
    "{{ invoice_date }}", "{{ prev_month_start }}", "{{ prev_month_end }}",
    "{{ prev_month_name }}", "{{ prev_month_year }}", "{{ invoice_number }}",
    "{{ currency }}", "{{ client_name }}", "{{ value_original }}", "{{ value_brl }}",
    "{start_date}", "{end_date}", "{{ company_cnpj }}"
}

VALID_EMAIL_MACROS = VALID_INVOICE_MACROS | {
    "{{ error }}", "{{ month_info }}", "{{ failsafe_reason }}",
    "{{ nf_number }}", "{{ amount_brl }}", "{{ verification_code }}"
}
...
class InvoiceTemplateItemForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'unit_price_foreign': forms.NumberInput(attrs={'class': 'form-control', 'step': '0.01', 'min': '0.00'}),
        }
...
InvoiceTemplateItemFormSet = inlineformset_factory(
    InvoiceTemplate, InvoiceTemplateItem, form=InvoiceTemplateItemForm,
    extra=0, can_delete=True
)
...
class CompanySettingsForm(forms.ModelForm):
    ...
    opening_date = forms.DateField(
        ...
        widget=forms.DateInput(
            format='%d/%m/%Y',
            attrs={'type': 'text', 'readonly': 'readonly', 'class': 'form-control', 'style': 'pointer-events: none; background-color: #e9ecef;'}
        )
    )
    class Meta:
        model = CompanySettings
        exclude = ['debug_mode', 'pfx_cert_pem', 'pfx_key_pem', 'certificate_valid_until']
        labels = {
            'address_cep': 'CEP',
        ...
        widgets = {
            'company_name': forms.TextInput(attrs={'readonly': 'readonly', 'class': 'form-control', 'style': 'pointer-events: none; background-color: #e9ecef;'}),
            'address_city_ibge': forms.HiddenInput(),
            'address_tipo_logradouro': forms.TextInput(attrs={'readonly': 'readonly', 'class': 'form-control', 'style': 'pointer-events: none; background-color: #e9ecef;'}),
            'address_line1': forms.TextInput(attrs={'readonly': 'readonly', 'class': 'form-control', 'style': 'pointer-events: none; background-color: #e9ecef;'}),
            'address_cep': forms.TextInput(attrs={'readonly': 'readonly', 'class': 'form-control', 'style': 'pointer-events: none; background-color: #e9ecef;'}),
            'address_number': forms.TextInput(attrs={'readonly': 'readonly', 'class': 'form-control', 'style': 'pointer-events: none; background-color: #e9ecef;'}),
            'address_line2': forms.TextInput(attrs={'readonly': 'readonly', 'class': 'form-control', 'style': 'pointer-events: none; background-color: #e9ecef;'}),
            'address_neighborhood': forms.TextInput(attrs={'readonly': 'readonly', 'class': 'form-control', 'style': 'pointer-events: none; background-color: #e9ecef;'}),
            'address_uf': forms.TextInput(attrs={'readonly': 'readonly', 'class': 'form-control', 'style': 'pointer-events: none; background-color: #e9ecef;'}),
            'address_city': forms.TextInput(attrs={'readonly': 'readonly', 'class': 'form-control', 'style': 'pointer-events: none; background-color: #e9ecef;'}),
        }
    ...
    def clean(self):
        ...
        if pfx_file:
            if not pfx_password:
                self.add_error('pfx_password_input', 'A senha é obrigatória ao enviar um arquivo PFX.')
            else:
                try:
                    from core.nfse.cert import carregar_certificado_pfx_bytes
                    pfx_data = pfx_file.read()
                    key_pem, cert_pem, exp = carregar_certificado_pfx_bytes(pfx_data, pfx_password)
    ...
    def save(self, commit=True):
        ...
        if self.cleaned_data.get('pfx_cert_pem'):
            ...
            instance.certificate_valid_until = self.cleaned_data['certificate_valid_until']
        if commit:
            instance.save()
        return instance
    def clean_cnpj(self):
