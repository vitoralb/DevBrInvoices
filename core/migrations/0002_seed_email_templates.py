...
def seed_email_templates(apps, schema_editor):
    ...
    templates = [
        ...
        },
        {
            ...
            "name": "Invoice Emitida (Interno)",
            "subject": "Nova Invoice Emitida: {{ invoice_number }}",
            "body": "Uma nova invoice ({{ invoice_number }}) foi finalizada e emitida para {{ client_name }}.\n\nA invoice está em anexo.\nSe a NFS-e foi gerada, ela também está em anexo.",
            "language": "pt-br",
        },
        {
            "template_type": "COMPANY_CANCEL_FAILED",
            "name": "Falha no Cancelamento (Interno)",
            "subject": "ALERTA: Falha no Cancelamento da NFS-e - Invoice {{ invoice_number }}",
            "body": "O cancelamento da NFS-e para a invoice {{ invoice_number }} ({{ client_name }}) falhou após o número máximo de tentativas.\n\nErro: {{ error }}\n\nPor favor, acesse o sistema e verifique os logs.",
            "language": "pt-br",
        },
        {
            "template_type": "COMPANY_FAILSAFE_TRIGGERED",
            "name": "Failsafe Acionado (Interno)",
            "subject": "ALERTA: Failsafe bloqueou a emissão da NFS-e - Invoice {{ invoice_number }}",
            "body": "A invoice {{ invoice_number }} ({{ client_name }}) foi finalizada e enviada ao cliente com sucesso, porém a emissão automática da NFS-e foi cancelada pelo seguinte motivo:\n\n{{ failsafe_reason }}\n\nPor favor, acesse o sistema para emitir a NFS-e manualmente.",
            "language": "pt-br",
        },
        {
            ...
            "body": "Uma nova NFS-e ({{ nf_number }}) foi emitida para a invoice {{ invoice_number }} ({{ client_name }}).\n\nValor (BRL): R$ {{ amount_brl }}\nCódigo de Verificação: {{ verification_code }}\n\nA NFS-e está em anexo.",
            "language": "pt-br",
        },
        {
            "template_type": "COMPANY_NFSE_FAILED",
            "name": "Falha na Emissão da NFS-e (Interno)",
            "subject": "ALERTA: Falha na Emissão da NFS-e - Invoice {{ invoice_number }}",
            "body": "A emissão da NFS-e para a invoice {{ invoice_number }} ({{ client_name }}) falhou após o número máximo de tentativas.\n\nErro: {{ error }}\n\nPor favor, acesse o sistema e tente emitir a NFS-e manualmente novamente.",
            "language": "pt-br",
        },
    ]
